...
task Init {
    ...
    ##### New content - Installing the required modules before building the module
    Write-Verbose -Message "Initializing Required Modules"
    try {
        $RequriedModules = Test-ModuleManifest ./Source/*.psd1 | Select-Object RequiredModules
        Write-Verbose -Message "Required Modules where found, checking if download is needed"
        foreach ($requiredModule in $RequriedModules.RequiredModules){
            if (-not(Get-Module -Name $requiredModule -ListAvailable)){
                Write-Warning "Required module '$($requiredModule)' is missing or out of date. Installing module now."
                Install-Module -Name $requiredModule -Scope CurrentUser -Force
            }
        } 
    }
    catch {
        Write-Verbose -Message "Could not load RequiredModules"
    }
}
